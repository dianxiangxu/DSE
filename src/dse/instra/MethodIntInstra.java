...
public class MethodIntInstra extends BodyTransformer {
	...
	static{
		...
		collectorClass = Scene.v().getSootClass("dse.instra.SootIntCollectorInstra");
	...
	protected void internalTransform(Body b, String phaseName, Map options) {
		...
		if(className.getName().contains("instra. ")){
...
//				}
		...
		if(method.getSignature().contains("void main(java.lang.String[])")){
			System.out.println("main method : "+method.getSignature());
			Chain<Unit> val = insertMapAfter.get(lastIdentity);
			val.addAll(shutdown(newLocalVars));
			insertMapAfter.put(lastIdentity, val);
			
			addToMap(insertMapAfter,lastIdentity,shutdown(newLocalVars));
		}
...
//
//			SootClass shutdownClass = Scene.v().getSootClass("dse.instra.ShutdownInstra");
//			Local runtime =  new JimpleLocal("runtime", RefType.v("java.lang.Runtime"));
//			Local shutdown =  new JimpleLocal("shutdown", RefType.v("dse.instra.Shutdown"));
...
//
//			JNewExpr shutdownNew = new JNewExpr(RefType.v("dse.instra.Shutdown"));
...
//	}
		...
		Chain<Unit> shutdownChain = new PatchingChain<Unit>(new HashChain<Unit>());
			...
			SootClass shutdownClass = Scene.v().getSootClass("dse.instra.ShutdownInstra");
			Local runtime =  new JimpleLocal("runtime", RefType.v("java.lang.Runtime"));
			Local shutdown =  new JimpleLocal("shutdown", RefType.v("dse.instra.ShutdownInstra"));
			newLocalVars.add(runtime);
			newLocalVars.add(shutdown);
			//do assignments of these locals
			SootMethod runtimeMethod = Scene.v().getMethod("<java.lang.Runtime: java.lang.Runtime getRuntime()>");
			...
			JNewExpr shutdownNew = new JNewExpr(RefType.v("dse.instra.ShutdownInstra"));
			Unit shutdownAssign = new JAssignStmt(shutdown, shutdownNew);
